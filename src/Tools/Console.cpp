...
void Console::Render(Renderer* renderer, float deltaTime)
{
    ...
    auto characterSize = _fontSettings.spriteFont->Get().CharacterDimension(_fontSettings.scale);
    ...
    while (y - characterSize.y > 0)
    {
        ...
        auto stringSize = _fontSettings.spriteFont->Get().MeasureStringWithNewlines(wrappedLine.c_str(), _fontSettings.scale);
        y -= stringSize.y;
