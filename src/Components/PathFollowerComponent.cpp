...
void PathFollowerComponent::FollowFlowField()
{
	...
	if (flowField != nullptr)
	{
		...
		for (auto p : points)
		{
			...
			if (scene->Raycast(p, flowField->target, result, false, [=](auto collider) { return collider.OwningEntity() !=
																								owner; })
				&& (state != PathFollowerState::FollowingEntity || result.handle.OwningEntity() != entityToFollow.GetValueOrNull()))
			{
				useBeeLine = false;
				break;
			}
