...
constexpr unsigned int CrcTable[] =
{
...
constexpr unsigned int Crc32Constexpr(const char* str)
{
	unsigned int crc = 0;
...
constexpr unsigned int Crc32Constexpr(const char* begin, const char* end)
{
	unsigned int crc = 0;

	while (begin < end)
	{
		const unsigned int byte = *begin++;
		const unsigned int index = (crc ^ (byte << 24)) >> 24;
